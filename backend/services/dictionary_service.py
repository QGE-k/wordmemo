...
# "希腊字母表的第X个字母"，这类长描述不符合"核心义"要求，
# 改为更直观的"α（阿尔法）"形式，与主流背单词软件（百词斩等）一致。
# key 同时支持中文大写序号（一/二/...）与阿拉伯数字（1/2/...）。
_GREEK_SYMBOLS = {
    '一': 'α（阿尔法）', '1': 'α（阿尔法）',
    '二': 'β（贝塔）', '2': 'β（贝塔）',
    '三': 'γ（伽马）', '3': 'γ（伽马）',
    ...
    def _clean_meaning(self, translation, pos_field=''):
        ...
        for line in clean_lines:
            ...
            for part in parts:
                ...
                if '希腊' in part:
                    greek_m = None
                    for gpat in (r'第\s*([一二三四五六七八九十]+|\d+)\s*个?字母',
                                 r'([一二三四五六七八九十]+|\d+)\s*个?字母'):
                        gm = _re.search(gpat, part)
                        if gm:
                            greek_m = gm
                            break
                    if greek_m:
                        symbol = _GREEK_SYMBOLS.get(greek_m.group(1).replace(' ', ''))
                        if symbol:
                            part = symbol
                # 过滤过长的释义（超过15个字的专业术语）
                if len(part) > 15:
